...
class AuthGuard {
    ...
    canActivateChild(next, state) {
        const /** @type {?} */ response = this.extractApiResponse(next.fragment);
        if (response) {
    ...
    extractApiResponse(fragment) {
        if (!!fragment) {
            return /** @type {?} */ (fromPairs(fragment.split('&').map((s) => s.split('='))));
        }
        return null;
    }
}
...
class SpotifyAuthInterceptor {
     ...
     */
    intercept(req, next) {
        const /** @type {?} */ authReq = req.clone({ setHeaders: this.tokenSvc.authHeader });
        return next.handle(authReq)._do((event) => { }, this.doOnError);
    }
}
...
export { ScopesBuilder, TokenService, AuthService, AuthGuard, SpotifyAuthInterceptor, SpotifyAuthModule, SpotifyAuthComponent as ɵa };
